# Refactor Pass 2 — Plan

**Branch:** `dev/grace-slop-refactor`  
**Prereq:** Pass 1 committed at `e670c06` and reviewed in `REFACTOR_PASS_1_REPORT_REVIEW.md`
...
## Part 1 — Pass 1 Carry-over Items

### P2-A: CSS Refactor (was C13+C14)

**Prereq:** Grace supplies screenshot baselines of all pages and UI states before any file is touched.

**Scope:**
- Split `App.css` (822 lines) into per-page/component stylesheets under `frontendReactJs/src/styles/`
- Extract all 122 inline `style={}` props from 14 tsx files into those CSS files
- Zero inline styles should remain after this pass

**Target file structure:**
```
frontendReactJs/src/styles/
  global.css          ← resets, CSS variables, typography (from App.css and index.css)
  auth.css            ← AuthPage.tsx
  chat.css            ← ChatsPage.tsx, ChatView.tsx
  settings.css        ← SettingsPage.tsx
  components/
    link-wizard.css
    onboarding.css
    timeline.css
    connections-panel.css
    connect-telegram.css
    connect-whatsapp.css
    find-contact.css
    key-manager.css
    app-dialogs.css   ← dialog overlays, modals, nuke countdown from App.tsx
```

**Execution rules (mandatory):**
1. Screenshot baseline of every page and UI state before touching any file
2. Grep each class name to confirm it belongs to the component being extracted — no guessing
3. Move rules verbatim; no edits during the move pass
4. Extract inline styles one component at a time, simplest first, `App.tsx` last
5. Visual check after every file
6. `cd frontendReactJs && npx tsc --noEmit` after the complete pass
