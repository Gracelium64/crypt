...
- Realtime: Socket.IO push for new messages; polling fallback supported.
...
2. Key generation & registration
   ...
   - Public key is exported (raw → base64) and can be registered to the public directory via `/api/keys/register`.
   - Private key is stored only in browser localStorage under `crypt:priv:<ownerId>`.
...
3. Provider link (LINK <code>)
   ...
   - The provider (telegram/whatsapp) side must send the `LINK <code>` message or the operator can complete the link via the admin webhook. The backend exposes `/api/provider/link/status/:code` for polling.
