...
uploadsRouter.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      console.error("[uploads/base64]", error);
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    form.parse(req, async (err: any, fields: any, files: any) => {
      ...
      try {
        ...
        // Client may request "raw" for encrypted blobs, default to "image"
        const requestedType = String((fields as any)?.resourceType ?? "image");
        const resourceType = (ALLOWED_RESOURCE_TYPES.has(requestedType) ? requestedType : "image") as "image" | "raw";
        ...
        console.error("[uploads/formidable]", uploadErr);
        res.status(500).json({ ok: false, error: "internal server error" });
        return;
      }
    });
